...
public abstract class ExtendedBehaviour<E extends LivingEntity> extends Behavior<E> {
    ...
    /**
     * A callback for when the task begins. Use this to trigger effects or handle things when the entity activates this
     * task.
     *
    ...
    /**
     * Set the length that the task should wait for between activations. This is the time between when the task stops,
     * and it is able to start again. The value used is in <i>ticks</i>
     *
     * @param timeProvider A function for the tick value
     * @return this
     */
    ...
    /**
     * Set an additional condition for the behaviour to be able to start. Useful for dynamically predicating
     * behaviours.<br>
    ...
    /**
     * Set an automatic condition for the behavior to stop. Useful for dynamically stopping behaviours. Has no effect on
     * one-shot behaviours that don't have a runtime.<br>
     ...
     * <p>
     * Functionally, this means that the behaviour will not timeout, and will rely on other factors to stop (such as
     * memory conditions failing)
     * </p>
     *
     * @return this
     */
     ...
     * By this stage, memory conditions from {@link ExtendedBehaviour#getMemoryRequirements()} have already been
     * checked.
     *
    ...
    /**
     * The root stop method for when this behaviour stops. This method should only be overridden by other abstract
     * subclasses. <br>
    ...
    /**
     * Override this for custom behaviour implementations. This is a safe endpoint for behaviours so that all required
     * auto-handling is safely contained without super calls.<br>
     * This is called when the behaviour is to start. Set up any instance variables needed or perform the required
     * actions.<br>
     * By this stage any memory requirements set in {@link ExtendedBehaviour#getMemoryRequirements()} are true, so any
     * memories paired with {@link MemoryStatus#VALUE_PRESENT} are safe to retrieve.
     *
     ...
     * Override this for custom behaviour implementations. This is a safe endpoint for behaviours so that all required
     * auto-handling is safely contained without super calls.<br>
     * This is called when the behaviour is to stop. Close off any instanced variables and such here, ready for the next
     * start.
     *
     * @param entity The entity being handled (I.E. the owner of the brain)
     */
    ...
    /**
     * The root method to check if this behaviour should continue running. This method should only be overridden by
     * other abstract subclasses.<br>
     * If overriding, ensure you either call super or manually call the {@link ExtendedBehaviour#stopCondition} check
     * yourself.
     *
     * @param level    The level the entity is in
     * @param entity   The entity the brain belongs to
    ...
    /**
     * Check whether the behaviour should continue running. This is checked before {@link ExtendedBehaviour#tick(E)}.
     * <br>
     * Memories are not guaranteed to be in their required state here, so if you have required memories, it might be
     * worth checking them here.
     *
     * @param entity The owner of the brain
     * @return Whether the behaviour should continue ticking
     */
    ...
    /**
     * The root tick method for when this behaviour ticks. This method should only be overridden by other abstract
     * subclasses. <br>
     ...
     * Override this for custom behaviour implementations. This is a safe endpoint for behaviours so that all required
     * auto-handling is safely contained without super calls.<br>
     * This is called when the behaviour is ticked. Be aware this is called <i>every tick</i>, so use tick reduction if
     * needed to minimise performance impacts of goals. <br>
     * NOTE: Memory requirements are <i>not</i> guaranteed at this stage. If you are retrieving brain memories, you'll
     * need to check their presence before use.
     *
     * @param entity The entity being handled (I.E. the owner of the brain)
     */
    ...
    /**
     * The list of memory requirements this task has prior to starting. This outlines the approximate state the brain
     * should be in, in order to allow this behaviour to run. <br>
     * Bonus points if it's a statically-initialised list.
     *
     * @return The {@link List} of {@link MemoryModuleType Memories} and their associated required {@link MemoryStatus
     *         status}
     */
