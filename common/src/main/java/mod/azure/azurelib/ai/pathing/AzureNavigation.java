...
public class AzureNavigation extends GroundPathNavigation {
    ...
    /**
     * Ensures a minimum width of 1.0 for entities, addressing an issue where smaller entities (less than 0.8 units in
     * width) encounter pathfinding failures. This resolves bugs such as MC-226637, where small entities end up
     * "spinning" due to improper navigation logic.
     * <p>
     * By enforcing a minimum calculated width of 1.0, this method prevents the pathfinding system from failing on
     * smaller entities, while leaving the behavior of larger entities unchanged. It may also reduce performance
     * overhead by preventing frequent hitbox and AI updates caused by entity spinning.
     *
     * @author Modrome
     * @return the maximum of the entity's actual width and 1.0, ensuring a minimum width for correct pathing.
     */
    public float getMinimumWidth() {
        return Math.max(this.mob.getBbWidth(), 1.0F); // Return whichever value is greater, for small entities, this
                                                      // returns 1.0 no matter what, fixing our spinning entities.
    }
            ...
            for (int x = x0; x != x1; x += stepx) {
                for (int z = z0; z != z1; z += stepz) {
                    ...
                    if (below == BlockPathTypes.WATER || below == BlockPathTypes.LAVA || below == BlockPathTypes.OPEN)
                        return false;
                    BlockPathTypes in = this.nodeEvaluator.getBlockPathType(
                        this.level,
                        x,
                        y0,
                        z,
                    ...
                    if (priority < 0.0F || priority >= 8.0F)
                        return false;
                    if (
                        in == BlockPathTypes.DAMAGE_FIRE || in == BlockPathTypes.DANGER_FIRE
