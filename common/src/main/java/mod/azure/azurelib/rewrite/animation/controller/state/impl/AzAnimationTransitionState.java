...
public final class AzAnimationTransitionState<T> extends AzAnimationState<T> {
    ...
    /**
     * Invoked when the transition state is entered in the animation state machine. This method performs
     * the necessary setup for transitioning between animations, including calling the superclass's
     * `onEnter` method and invoking the `prepareTransition` method to initialize the transition process.
     *
     * @param context the context of the animation state machine, providing access to the animation
     *                controller, animation context, and references to the state machine
     */
    @Override
    ...
    /**
     * Updates the transition state of the animation controller. This method checks whether the transition period has been
     * completed and determines the appropriate next action, such as transitioning to the play state or continuing the
     * transition by updating keyframes.
     *
    ...
    /**
     * Prepares the animation transition process by resetting the necessary components, updating callbacks,
     * caching bone snapshots, and setting the next animation in the animation controller.
     *
     * @param context The context of the animation controller state machine, providing the animation context,
     *                animation controller, and references required for processing the transition.
     */
