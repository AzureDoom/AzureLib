...
public class S2C_SendConfigData extends AbstractPacket {
    ...
    public static S2C_SendConfigData receive(PacketBuffer buffer) {
        ...
        ConfigHolder.getConfig(config).ifPresent(data -> {
            ...
            for (int j = 0; j < i; j++) {
                ...
                if (value == null) {
                    AzureLib.LOGGER.fatal(AzureLibNetwork.MARKER, "Received unknown config value {}", fieldId);
    ...
    private static <V> void setValue(ConfigValue<V> value, PacketBuffer buffer) {
        TypeAdapter adapter = value.getAdapter();
