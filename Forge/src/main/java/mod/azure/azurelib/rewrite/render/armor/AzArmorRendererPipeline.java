...
public class AzArmorRendererPipeline extends AzRendererPipeline<ItemStack> {
    ...
    public void scaleModelForBaby(AzArmorRendererPipelineContext context, boolean isReRender) {
        ...
        if (currentSlot == EquipmentSlotType.HEAD) {
            if (baseModel.isChildHeadScaled) {
                float headScale = 1.5f / baseModel.childHeadScale;

                poseStack.scale(headScale, headScale, headScale);
            }

            poseStack.translate(0, baseModel.childBodyOffsetY / 16f, baseModel.childHeadOffsetZ / 16f);
        } else {
            float bodyScale = 1 / baseModel.childBodyScale;

            poseStack.scale(bodyScale, bodyScale, bodyScale);
            poseStack.translate(0, baseModel.childBodyOffsetY / 16f, 0);
        }
    }
