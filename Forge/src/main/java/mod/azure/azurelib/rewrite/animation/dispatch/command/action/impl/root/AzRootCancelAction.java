...
public class AzRootCancelAction implements AzAction {
    ...
    public static final Function<PacketBuffer, AzRootCancelAction> DECODER = buf -> {
        String controllerName = buf.readString(); // Read UTF-8 string for the controller's name
        return new AzRootCancelAction(controllerName);
    };

    public static final BiConsumer<PacketBuffer, AzRootCancelAction> ENCODER = (buf, action) -> {
    ...
    public static AzRootCancelAction decode(PacketBuffer buf) {
        return DECODER.apply(buf); // Delegate to the DECODER functional interface
    }

    public static void encode(PacketBuffer buf, AzRootCancelAction action) {
