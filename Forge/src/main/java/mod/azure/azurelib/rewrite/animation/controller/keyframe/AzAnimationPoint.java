...
/**
 * Animation state class that holds the state of an animation at a given point. Represents: - A specific animation tick
 * - Transition duration - Animation start and end values - Associated keyframe (optional)
 */
public class AzAnimationPoint {
    ...
    public boolean equals(Object o) {
        ...
        if (Double.compare(that.currentTick, currentTick) != 0)
            return false;
        if (Double.compare(that.transitionLength, transitionLength) != 0)
            return false;
        if (Double.compare(that.animationStartValue, animationStartValue) != 0)
            return false;
        if (Double.compare(that.animationEndValue, animationEndValue) != 0)
            return false;
