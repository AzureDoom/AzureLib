...
public class AzArmorLayer<T extends LivingEntity> implements AzRenderLayer<T> {
    ...
    public void preRender(AzRendererPipelineContext<T> context) {
        this.mainHandStack = context.animatable().getItemBySlot(EquipmentSlotType.MAINHAND);
        this.offhandStack = context.animatable().getItemBySlot(EquipmentSlotType.OFFHAND);
        this.helmetStack = context.animatable().getItemBySlot(EquipmentSlotType.HEAD);
        this.chestplateStack = context.animatable().getItemBySlot(EquipmentSlotType.CHEST);
        this.leggingsStack = context.animatable().getItemBySlot(EquipmentSlotType.LEGS);
        this.bootsStack = context.animatable().getItemBySlot(EquipmentSlotType.FEET);
    }
    ...
    protected EquipmentSlotType getEquipmentSlotForBone(
        ...
        for (EquipmentSlotType slot : EquipmentSlotType.values()) {
            boolean isHumanoidArmorSlotType = slot.getType() == EquipmentSlotType.Group.ARMOR;
    ...
    protected void prepModelPartForRender(AzRendererPipelineContext<T> context, AzBone bone, ModelRenderer sourcePart) {
        ...
        ModelRenderer.ModelBox armorCube = sourcePart.cubes.get(0);
     ...
     * <p>
     * Equivalent to {@link net.minecraft.client.renderer.entity.layers.BipedArmorLayer#getArmorLocation
