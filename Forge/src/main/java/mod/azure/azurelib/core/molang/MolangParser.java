...
public class MolangParser extends MathBuilder {
    ...
    protected static MolangValue parseOneLine(
        ...
        try {
            ...
            if (
                symbols.size() >= 3 && symbols.get(0) instanceof String && INSTANCE.isVariable(symbols.get(0))
                ...
                if (
                    !VARIABLES.containsKey(String.valueOf(symbols.get(0))) && !currentStatement.locals.containsKey(
                        String.valueOf(symbols.get(0))
                    )
                ) {
                    currentStatement.locals.put(
                        String.valueOf(symbols.get(0)),
                        (variable = new LazyVariable(String.valueOf(symbols.get(0)), 0))
                    );
                } else {
