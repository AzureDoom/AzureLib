...
public class AzureNavigation extends GroundPathNavigator {
    ...
    protected void followThePath() {
        ...
        if (
            this.tryShortcut(
                path,
                new Vector3d(this.mob.getX(), this.mob.getY(), this.mob.getZ()),
                pathLength,
                base,
                max
            )
    ...
    private boolean isAt(Path path, float threshold) {
        ...
        return MathHelper.abs((float) (this.mob.getX() - pathPos.x)) < threshold && MathHelper.abs(
        ...
        do {
            ...
            for (int x = x0; x != x1; x += stepx) {
                for (int z = z0; z != z1; z += stepz) {
                    ...
                    PathNodeType below = this.nodeEvaluator.getBlockPathType(
                        this.level,
                        x,
                        y0 - 1,
                        z,
                    ...
                    if (below == PathNodeType.WATER || below == PathNodeType.LAVA || below == PathNodeType.OPEN)
                        return false;
                    PathNodeType in = this.nodeEvaluator.getBlockPathType(
                        this.level,
                        x,
                        y0,
                        z,
