...
public class AzureNavigation extends GroundPathNavigator {
    ...
    protected void pathFollow() {
        ...
        if (
            this.tryShortcut(
                path,
                new Vec3d(this.entity.getPosX(), this.entity.getPosY(), this.entity.getPosZ()),
                pathLength,
                base,
                max
            )
            ...
            if (
                this.isAt(path, 0.5F) || this.atElevationChange(path) && this.isAt(path, this.entity.getWidth() * 0.5F)
            ) {
    ...
    public void tick() {
        ...
        if (this.noPath()) {
            if (this.pathToPosition != null) {
                if (
                    this.pathToPosition.withinDistance(this.entity.getPositionVec(), this.entity.getWidth())
                        || this.entity.getPosY() > (double) this.pathToPosition.getY() && new BlockPos(
                            this.pathToPosition.getX(),
                            this.entity.getPosY(),
                            this.pathToPosition.getZ()
                        ).withinDistance(this.entity.getPositionVec(), this.entity.getWidth())
                ) {
                    this.pathToPosition = null;
                } else {
        ...
        if (this.getTargetPos() != null)
            this.entity.getLookController()
                .setLookPosition(this.getTargetPos().getX(), this.getTargetPos().getY(), this.getTargetPos().getZ());
    }

    private boolean isAt(Path path, float threshold) {
        final Vec3d pathPos = path.getPosition(this.entity);
        return MathHelper.abs((float) (this.entity.getPosX() - pathPos.x)) < threshold && MathHelper.abs(
            (float) (this.entity.getPosZ() - pathPos.z)
        ) < threshold && Math.abs(this.entity.getPosY() - pathPos.y) < 1.0D;
    }
    ...
    private boolean atElevationChange(Path path) {
        ...
        final int end = Math.min(
            ...
            curr + MathHelper.ceil(this.entity.getWidth() * 0.5F) + 1
        );
        ...
        do {
            ...
            for (int x = x0; x != x1; x += stepx) {
                for (int z = z0; z != z1; z += stepz) {
                    ...
                    PathNodeType below = this.nodeProcessor.getPathNodeType(
                        this.world,
                        x,
                        y0 - 1,
                        z,
                    ...
                    if (below == PathNodeType.WATER || below == PathNodeType.LAVA || below == PathNodeType.OPEN)
                        return false;
                    PathNodeType in = this.nodeProcessor.getPathNodeType(
                        this.world,
                        x,
                        y0,
                        z,
                    ...
                    if (priority < 0.0F || priority >= 8.0F)
                        return false;
                    if (
                        in == PathNodeType.DAMAGE_FIRE || in == PathNodeType.DANGER_FIRE
                            || in == PathNodeType.DAMAGE_OTHER
                    )
                        return false;
                }
            }
