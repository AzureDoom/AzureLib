...
public abstract class BaseGunItem extends Item implements GeoItem {
    ...
    public static EntityRayTraceResult hitscanTrace(PlayerEntity player, double range, float ticks) {
        ...
        double traceDistance = player.level.clip(
            new RayTraceContext(start, end, RayTraceContext.BlockMode.COLLIDER, RayTraceContext.FluidMode.NONE, player)
        ).getLocation().distanceToSqr(end);
        for (
            Entity possible : player.level.getEntities(
                player,
            ...
            if (possible.getBoundingBox().inflate(0.3D).clip(start, end).isPresent())
                if (start.distanceToSqr(possible.getBoundingBox().inflate(0.3D).clip(start, end).get()) < traceDistance)
                    return ProjectileHelper.getEntityHitResult(
                        ...
                        (target) -> !target.isSpectator() && player.isAttackable() && player.canSee(target)
                    );
        }
        return null;
    }
