        ...
        new IMetadataSectionSerializer<GeoGlowingTextureMeta>() {
            ...
            private List<Pixel> fromSections(JsonArray sectionsArray) {
                ...
                for (JsonElement element : sectionsArray) {
                    ...
                    int x1 = JSONUtils.getAsInt(
                        (JsonObject) element,
                        "x1",
                        JSONUtils.getAsInt((JsonObject) element, "x", 0)
                    );
                    int y1 = JSONUtils.getAsInt(
                        (JsonObject) element,
                        "y1",
                        JSONUtils.getAsInt((JsonObject) element, "y", 0)
                    );
                    int x2 = JSONUtils.getAsInt(
                        (JsonObject) element,
                        "x2",
                        JSONUtils.getAsInt((JsonObject) element, "w", 0) + x1
                    );
                    int y2 = JSONUtils.getAsInt(
                        (JsonObject) element,
                        "y2",
                        JSONUtils.getAsInt((JsonObject) element, "h", 0) + y1
                    );
                    int alpha = JSONUtils.getAsInt(
                        (JsonObject) element,
                        "alpha",
                        JSONUtils.getAsInt((JsonObject) element, "a", 0)
                    );
