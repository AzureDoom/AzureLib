...
/**
 * The {@link GeoAnimatable} interface specific to {@link Entity Entities}. This also applies to Projectiles and other
 * Entity subclasses.<br>
 * <b>NOTE:</b> This <u>cannot</u> be used for entities using the
 * {@link mod.azure.azurelib.renderer.GeoReplacedEntityRenderer} as you aren't extending {@code Entity}. Use
 * {@link GeoReplacedEntity} instead.
 */
@Deprecated()
public interface GeoEntity extends GeoAnimatable {
    ...
    default <D> void setAnimData(SerializableDataTicket<D> dataTicket, D data) {
        ...
        if (entity.getEntityWorld().isRemote()) {
            ...
            AzureLibNetwork.send(
                new EntityAnimDataSyncPacket<>(entity.getEntityId(), dataTicket, data),
    ...
    default void triggerAnim(@Nullable String controllerName, String animName) {
        ...
        if (entity.getEntityWorld().isRemote()) {
            getAnimatableInstanceCache().getManagerForId(entity.getEntityId())
                .tryTriggerAnimation(controllerName, animName);
        } else {
            AzureLibNetwork.send(
                new EntityAnimTriggerPacket<>(entity.getEntityId(), controllerName, animName),
                PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> entity)
            );
        }
    }
