...
public interface GeoReplacedEntity extends SingletonGeoAnimatable {
    ...
    default <D> void setAnimData(Entity relatedEntity, SerializableDataTicket<D> dataTicket, D data) {
        if (relatedEntity.getEntityWorld().isRemote()) {
            ...
            AzureLibNetwork.send(
                new EntityAnimDataSyncPacket<>(relatedEntity.getEntityId(), dataTicket, data),
    ...
    default void triggerAnim(Entity relatedEntity, @Nullable String controllerName, String animName) {
        if (relatedEntity.getEntityWorld().isRemote()) {
            getAnimatableInstanceCache().getManagerForId(relatedEntity.getEntityId())
                .tryTriggerAnimation(controllerName, animName);
        } else {
            AzureLibNetwork.send(
                new EntityAnimTriggerPacket<>(relatedEntity.getEntityId(), controllerName, animName),
                PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> relatedEntity)
            );
        }
    }
