...
/**
 * {@link GeoRenderLayer} for rendering {@link BlockState BlockStates} or {@link ItemStack ItemStacks} on a given
 * {@link GeoAnimatable}
 */
@Deprecated()
    ...
    protected void renderStackForBone(
        ...
        if (animatable instanceof LivingEntity) {
            Minecraft.getInstance()
                .getItemRenderer()
                .renderStatic(
                    ((LivingEntity) animatable),
                    stack,
                    getTransformTypeForStack(bone, stack, animatable),
                    false,
                    poseStack,
                    bufferSource,
                    ((LivingEntity) animatable).level,
                    packedLight,
                    packedOverlay
                );
        } else {
