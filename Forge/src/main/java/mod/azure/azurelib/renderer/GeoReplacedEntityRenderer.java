...
public class GeoReplacedEntityRenderer<E extends Entity, T extends GeoAnimatable> extends EntityRenderer<E> implements GeoRenderer<T> {
    ...
    public void actuallyRender(
        ...
        LivingEntity livingEntity = this.currentEntity instanceof LivingEntity
            ? (LivingEntity) this.currentEntity
            : null;

        boolean shouldSit = this.currentEntity.isPassenger() && (this.currentEntity.getRidingEntity() != null
            && this.currentEntity.getRidingEntity().shouldRiderSit());
        ...
        if (shouldSit && this.currentEntity.getRidingEntity() instanceof LivingEntity) {
            lerpBodyRot = MathHelper.rotLerp(
                partialTick,
                ((LivingEntity) currentEntity.getRidingEntity()).prevRenderYawOffset,
        ...
        float headPitch = MathHelper.lerp(
            partialTick,
            this.currentEntity.prevRotationPitch,
            this.currentEntity.rotationPitch
        );
    ...
    protected void applyRotations(
        ...
        if (this.currentEntity instanceof LivingEntity) {
            if (((LivingEntity) this.currentEntity).deathTime > 0) {
                ...
                poseStack.rotate(
                    Vector3f.YP.rotationDegrees((((LivingEntity) this.currentEntity).ticksExisted + partialTick) * -75f)
                );
     ...
     * {@link net.minecraft.entity.monster.SpiderEntity Spider} flipping upside down on death.<br>
    ...
    public boolean canRenderName(E entity) {
        ...
        if (
            entity instanceof MobEntity && (!entity.getAlwaysRenderNameTagForRender() && (!entity.hasCustomName()
                || entity != this.renderManager.pointedEntity))
        )
            return false;
        ...
        if (entityTeam == null)
            return Minecraft.isGuiEnabled() && entity != minecraft.getRenderViewEntity() && visibleToClient && !entity
                .isBeingRidden();
        ...
        return tagVisibiility == Visible.ALWAYS
            ? visibleToClient
            : tagVisibiility == Visible.HIDE_FOR_OTHER_TEAMS
                ? playerTeam == null
                    ? visibleToClient
                    : entityTeam.isSameTeam(
                        playerTeam
                    ) && (entityTeam.getSeeFriendlyInvisiblesEnabled() || visibleToClient)
                : tagVisibiility == Visible.HIDE_FOR_OWN_TEAM && (playerTeam == null
                    ? visibleToClient
                    : !entityTeam.isSameTeam(
                        playerTeam
                    ) && visibleToClient);
     ...
     * Gets a packed overlay coordinate pair for rendering.<br>
     * Mostly just used for the red tint when an entity is hurt, but can be used for other things like the
     * {@link net.minecraft.entity.monster.CreeperEntity} white tint when exploding.
     */
    @Override
     ...
     * duplicated here for flexible usage
     */
    public <H extends Entity, M extends MobEntity> void renderLeash(
        M mob,
        float partialTick,
        MatrixStack poseStack,
        IRenderTypeBuffer bufferSource,
        ...
        for (int segment = 0; segment <= 24; ++segment) {
            GeoReplacedEntityRenderer.renderLeashPiece(
                ivertexbuilder,
                matrix4f,
                f,
                f1,
