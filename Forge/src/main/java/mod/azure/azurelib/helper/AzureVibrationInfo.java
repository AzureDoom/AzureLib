...
public record AzureVibrationInfo(
    GameEvent gameEvent,
    float distance,
    Vec3 pos,
    @Nullable UUID uuid,
    ...
    public static final Codec<AzureVibrationInfo> CODEC = RecordCodecBuilder.create(
        instance -> instance
            .group(
                (BuiltInRegistries.GAME_EVENT.byNameCodec().fieldOf("game_event"))
                    .forGetter(AzureVibrationInfo::gameEvent),
                (Codec.floatRange(0.0f, Float.MAX_VALUE).fieldOf("distance")).forGetter(AzureVibrationInfo::distance),
            ...
            .apply(
                instance,
                (gameEvent, float_, vec3, optional, optional2) -> new AzureVibrationInfo(
                    (GameEvent) gameEvent,
                    float_.floatValue(),
                    (Vec3) vec3,
    ...
    public AzureVibrationInfo(GameEvent gameEvent, float f, Vec3 vec3, @Nullable Entity entity) {
        this(
            ...
            AzureVibrationInfo.getProjectileOwner(entity),
            entity
        );
    }
