    ...
    public ExampleEntityRenderer(EntityRendererProvider.Context context) {
        super(
                AzEntityRendererConfig.<ExampleEntity>builder(MODEL_LOCATION, TEXTURE_LOCATION)
                        .addRenderLayer(new AzBlockAndItemLayer<UUID, ExampleEntity>() {
...
2. **`getTransformTypeForStack`**:
   ...
   - For example:
     - `ItemTransforms.TransformType.THIRD_PERSON_RIGHT_HAND` for the right hand.
     - `ItemTransforms.TransformType.THIRD_PERSON_LEFT_HAND` for the left hand.

   ```java
   @Override
   protected ItemTransforms.TransformType getTransformTypeForStack(AzBone bone, ItemStack stack, ExampleEntity animatable) {
       return switch (bone.getName()) {
           case "rightHandBone" -> ItemTransforms.TransformType.THIRD_PERSON_RIGHT_HAND;
           case "leftHandBone" -> ItemTransforms.TransformType.THIRD_PERSON_LEFT_HAND;
