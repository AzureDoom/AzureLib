...
/**
 * {@link GeoRenderLayer} for rendering
 * {@link net.minecraft.world.level.block.state.BlockState BlockStates} or
 * {@link net.minecraft.world.item.ItemStack ItemStacks} on a given
 * {@link GeoAnimatable}
 */
public class BlockAndItemGeoLayer<T extends GeoAnimatable> extends GeoRenderLayer<T> {
	...
	public BlockAndItemGeoLayer(GeoRenderer<T> renderer, BiFunction<GeoBone, T, ItemStack> stackForBone,
	...
	/**
	 * Return an ItemStack relevant to this bone for rendering, or null if no
	 * ItemStack to render
	 */
	@Nullable
	...
	/**
	 * Return a BlockState relevant to this bone for rendering, or null if no
	 * BlockState to render
	 */
	@Nullable
	...
	/**
	 * Return a specific TransFormType for this {@link ItemStack} render for this
	 * bone.
	 */
	...
	/**
	 * This method is called by the {@link GeoRenderer} for each bone being
	 * rendered.<br>
	 * This is a more expensive call, particularly if being used to render something
	 * on a different buffer.<br>
	 * It does however have the benefit of having the matrix translations and other
	 * transformations already applied from render-time.<br>
	 ...
	 * <br>
	 * If you <i>do</i> use it, and you render something that changes the
	 * {@link VertexConsumer buffer}, you need to reset it back to the previous
	 * buffer using {@link MultiBufferSource#getBuffer} before ending the method
	 */
	@Override
	...
	protected void renderStackForBone(PoseStack poseStack, GeoBone bone, ItemStack stack, T animatable,
		...
		if (animatable instanceof LivingEntity livingEntity) {
			Minecraft.getInstance().getItemRenderer().renderStatic(livingEntity, stack,
					...
					livingEntity.level, packedLight, packedOverlay, livingEntity.getId());
		} else {
	 ...
	 */
	protected void renderBlockForBone(PoseStack poseStack, GeoBone bone, BlockState state, T animatable,
			MultiBufferSource bufferSource, float partialTick, int packedLight, int packedOverlay) {
		poseStack.pushPose();
