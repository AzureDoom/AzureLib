...
public class GeoReplacedEntityRenderer<E extends Entity, T extends GeoAnimatable> extends EntityRenderer<E> implements GeoRenderer<T> {
    ...
    public void actuallyRender(
        ...
        if (shouldSit && this.currentEntity.getVehicle() instanceof LivingEntity) {
            lerpBodyRot = Mth.rotLerp(
                partialTick,
                ((LivingEntity) this.currentEntity.getVehicle()).yBodyRotO,
                ((LivingEntity) this.currentEntity.getVehicle()).yBodyRot
            );
    ...
    protected void applyRotations(
        ...
        if (this.currentEntity instanceof LivingEntity) {
            if (((LivingEntity) this.currentEntity).deathTime > 0) {
                ...
                poseStack.mulPose(
                    Vector3f.ZP.rotationDegrees(Math.min(Mth.sqrt(deathRotation), 1) * getDeathMaxRotation(animatable))
                );
                ...
                poseStack.mulPose(
                    Vector3f.YP.rotationDegrees((((LivingEntity) this.currentEntity).tickCount + partialTick) * -75f)
                );
    ...
    public static boolean isEntityUpsideDown(LivingEntity livingEntity) {
        if (livingEntity instanceof Player || livingEntity.hasCustomName()) {
            ...
            if ("Dinnerbone".equals(s) || "Grumm".equals(s)) {
                return !(livingEntity instanceof Player) || ((Player) livingEntity).isModelPartShown(
    ...
    public boolean shouldShowName(E entity) {
        ...
        return tagVisibiility == Visibility.ALWAYS
            ? visibleToClient
            : tagVisibiility == Visibility.HIDE_FOR_OTHER_TEAMS
                ? playerTeam == null
                    ? visibleToClient
                    : entityTeam.isAlliedTo(playerTeam) && (entityTeam.canSeeFriendlyInvisibles() || visibleToClient)
                : tagVisibiility == Visibility.HIDE_FOR_OWN_TEAM
    ...
    public <H extends Entity, M extends Mob> void renderLeash(
        ...
        double lerpBodyAngle = (Mth.lerp(partialTick, mob.yBodyRotO, mob.yBodyRot) * ((float) Math.PI / 180))
            + 1.5707963267948966;
    ...
    public void fireCompileRenderLayersEvent() {
        GeoRenderEvent.ReplacedEntity.CompileRenderLayers.EVENT.invoker()
            .handle(new GeoRenderEvent.ReplacedEntity.CompileRenderLayers(this));
    }
    ...
    public boolean firePreRenderEvent(
        ...
        return GeoRenderEvent.ReplacedEntity.Pre.EVENT.invoker()
            .handle(
                new GeoRenderEvent.ReplacedEntity.Pre(this, poseStack, model, bufferSource, partialTick, packedLight)
            );
    }
    ...
    public void firePostRenderEvent(
        ...
        GeoRenderEvent.ReplacedEntity.Post.EVENT.invoker()
            .handle(
                new GeoRenderEvent.ReplacedEntity.Post(this, poseStack, model, bufferSource, partialTick, packedLight)
            );
    }
}
