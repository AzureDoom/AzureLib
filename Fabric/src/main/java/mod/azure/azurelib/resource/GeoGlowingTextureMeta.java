...
public class GeoGlowingTextureMeta {
    ...
    public static final MetadataSectionSerializer<GeoGlowingTextureMeta> DESERIALIZER =
        new MetadataSectionSerializer<GeoGlowingTextureMeta>() {
            ...
            private List<Pixel> fromSections(@Nullable JsonArray sectionsArray) {
                ...
                for (JsonElement element : sectionsArray) {
                    ...
                    int x1 = GsonHelper.getAsInt(
                        (JsonObject) element,
                        "x1",
                        GsonHelper.getAsInt((JsonObject) element, "x", 0)
                    );
                    int y1 = GsonHelper.getAsInt(
                        (JsonObject) element,
                        "y1",
                        GsonHelper.getAsInt((JsonObject) element, "y", 0)
                    );
                    int x2 = GsonHelper.getAsInt(
                        (JsonObject) element,
                        "x2",
                        GsonHelper.getAsInt((JsonObject) element, "w", 0) + x1
                    );
                    int y2 = GsonHelper.getAsInt(
                        (JsonObject) element,
                        "y2",
                        GsonHelper.getAsInt((JsonObject) element, "h", 0) + y1
                    );
                    int alpha = GsonHelper.getAsInt(
                        (JsonObject) element,
                        "alpha",
                        GsonHelper.getAsInt((JsonObject) element, "a", 0)
                    );
    ...
    public void createImageMask(NativeImage originalImage, NativeImage newImage) {
        for (Pixel pixel : this.pixels) {
            ...
            if (pixel.alpha > 0)
                color = NativeImage.combine(
                    pixel.alpha,
                    NativeImage.getB(color),
                    NativeImage.getG(color),
                    NativeImage.getR(color)
                );
