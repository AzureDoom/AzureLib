...
public class AzAutoGlowingLayer<T> implements AzRenderLayer<T> {
    ...
    public void render(AzRendererPipelineContext<T> context) {
        ...
        mod.azure.azurelib.render.AzRendererPipeline<T> renderPipeline = context.rendererPipeline();
        ...
        if (context.animatable() instanceof Entity) {
            ...
            boolean isInvisibleButVisibleToPlayer = entity.isInvisible() && !entity.isInvisibleTo(
                ClientUtils.getClientPlayer()
            );
            boolean shouldAppearGlowing = Minecraft.getInstance().crosshairPickEntity.isGlowing();
