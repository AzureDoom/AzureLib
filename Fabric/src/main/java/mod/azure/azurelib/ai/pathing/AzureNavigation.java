...
public class AzureNavigation extends GroundPathNavigation {
    ...
    protected void updatePath() {
        ...
        int pathLength = path.getSize();
        for (int i = path.getIndex(); i < path.getSize(); i++) {
            if (path.get(i).y != Math.floor(entityPos.y)) {
                pathLength = i;
                break;
            }
        }
        ...
        if (
            this.tryShortcut(
                path,
                new Vec3(this.mob.getX(), this.mob.getY(), this.mob.getZ()),
                pathLength,
                base,
                max
            )
    ...
    public Path createPath(Entity entity, int i) {
        this.pathToPosition = entity.getCommandSenderBlockPosition();
    ...
    public void tick() {
        ...
        if (this.isDone()) {
            if (this.pathToPosition != null) {
                if (
                    this.pathToPosition.closerThan(this.mob.position(), this.mob.getBbWidth()) || this.mob
                        .getY() > (double) this.pathToPosition.getY() && new BlockPos(
                            this.pathToPosition.getX(),
                            this.mob.getY(),
                            this.pathToPosition.getZ()
                        ).closerThan(this.mob.position(), this.mob.getBbWidth())
                ) {
                    this.pathToPosition = null;
                } else {
    ...
    private boolean isAt(Path path, float threshold) {
        final Vec3 pathPos = path.currentPos(this.mob);
    ...
    private boolean atElevationChange(Path path) {
        final int curr = path.getIndex();
        final int end = Math.min(path.getSize(), curr + Mth.ceil(this.mob.getBbWidth() * 0.5F) + 1);
        final int currY = path.get(curr).y;
        for (int i = curr + 1; i < end; i++) {
            if (path.get(i).y != currY) {
                return true;
            }
        }
        return false;
    ...
    private boolean tryShortcut(Path path, Vec3 entityPos, int pathLength, Vec3 base, Vec3 max) {
        for (int i = pathLength; --i > path.getIndex();) {
            final Vec3 vec = path.getPos(this.mob, i).subtract(entityPos);
