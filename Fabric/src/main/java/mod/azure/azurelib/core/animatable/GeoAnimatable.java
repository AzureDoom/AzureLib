...
/**
 * This is the root interface for all animatable objects in AzureLib. Generally speaking you should use one of the sub-interfaces relevant to your specific object so that your model can be automatically handled.<br>
 * See:<br>
 * <ul>
 * <li>{@code GeoBlock}</li>
...
public interface GeoAnimatable {
	 ...
	 * <br>
	 * Each controller can only play <u>one</u> animation at a time, and so animations that you intend to play concurrently should be handled in independent controllers. Note having multiple animations playing via multiple controllers can override parts of one animation with another if both animations use the same bones or child bones.
	 *
	...
	/**
	 * Each instance of a {@code GeoAnimatable} must return an instance of an {@link AnimatableInstanceCache}, which handles instance-specific animation info. Generally speaking, you should create your cache using {@code AzureLibUtil#createCache} and store it in your animatable instance, returning that cached instance when called.
	 * 
	...
	/**
	 * Defines the speed in which the {@link mod.azure.azurelib.core.animation.AnimationProcessor} should return {@link CoreGeoBone GeoBones} that currently have no animations to their default position.
	 */
