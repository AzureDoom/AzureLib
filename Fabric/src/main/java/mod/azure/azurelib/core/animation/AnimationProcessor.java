...
public class AnimationProcessor<T extends GeoAnimatable> {
    ...
    public Queue<QueuedAnimation> buildAnimationQueue(T animatable, RawAnimation rawAnimation) {
        ...
        for (RawAnimation.Stage stage : rawAnimation.getAnimationStages()) {
            ...
            if (animation == null) {
                System.out.println(
                    "Unable to find animation: " + stage.animationName() + " for " + animatable.getClass()
                        .getSimpleName()
                );
    ...
    /**
     * {@link Animation} and {@link mod.azure.azurelib.core.animation.Animation.LoopType} override pair, used to define
     * a playable animation stage for a {@link GeoAnimatable}
     */
