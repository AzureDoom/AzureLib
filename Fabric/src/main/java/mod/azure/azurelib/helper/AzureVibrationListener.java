...
public class AzureVibrationListener implements GameEventListener {
    ...
    public static Codec<AzureVibrationListener> codec(AzureVibrationListenerConfig vibrationListenerConfig) {
        return RecordCodecBuilder.create(
            instance -> instance
                .group(
                    (PositionSource.CODEC.fieldOf("source"))
                    ...
                    (ExtraCodecs.NON_NEGATIVE_INT.fieldOf("event_delay")).orElse(0)
                        .forGetter(
                ...
                .apply(
                    ...
                    (positionSource, integer, optional, vibrationSelector, integer2) -> new AzureVibrationListener(
                        ...
                        (AzureVibrationSelector) vibrationSelector,
                        (int) integer2
                    )
                )
        );
    ...
    public AzureVibrationListener(
        PositionSource positionSource,
        int i,
        AzureVibrationListenerConfig vibrationListenerConfig,
        @Nullable AzureVibrationInfo vibrationInfo,
        AzureVibrationSelector vibrationSelector,
        int j
    ) {
    ...
    public boolean handleGameEvent(ServerLevel level, GameEvent.Message eventMessage) {
        ...
        if (!this.config.shouldListen(level, this, new BlockPos(vec3), gameEvent, context))
            return false;
        if (AzureVibrationListener.isOccluded(level, vec3, vec32))
            return false;
    ...
    private static boolean isOccluded(Level level, Vec3 vec3, Vec3 vec32) {
        var vec33 = new Vec3(
            (double) Mth.floor(vec3.x) + 0.5,
            (double) Mth.floor(vec3.y) + 0.5,
            (double) Mth.floor(vec3.z) + 0.5
        );
        var vec34 = new Vec3(
            (double) Mth.floor(vec32.x) + 0.5,
            (double) Mth.floor(vec32.y) + 0.5,
    ...
    public static interface AzureVibrationListenerConfig {
        ...
        public void onSignalReceive(
            ...
            BlockPos var3,
            GameEvent var4,
            @Nullable Entity var5,
            @Nullable Entity var6,
            float var7
        );
