...
public class ConfigEntryWidget extends ContainerWidget implements WidgetAdder {
    ...
    public void renderWidget(GuiGraphics stack, int mouseX, int mouseY, float partialTicks) {
    	...
    	stack.drawString(font, this.getMessage(), this.getX(), (int)(this.getY() + (this.height - font.lineHeight) / 2.0F), 0xAAAAAA, true);
    ...
    public interface IDescriptionRenderer {
        void drawDescription(GuiGraphics stack, AbstractWidget widget, NotificationSeverity severity, List<FormattedCharSequence> text);
    }
}
